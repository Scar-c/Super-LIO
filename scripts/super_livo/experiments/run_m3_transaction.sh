...
for f in "$BAG" "$CFG" "$CAMCFG";do [ -f "$f" ]||{ FINAL_CLASS=CONFIG_FAIL;FINAL_REASON="missing $f";exit 1;};done
sha256sum "$CFG" "$CAMCFG">"$RUN_DIR/config_hashes.txt";write_state STATIC_PREFLIGHT_PASS "" "files present"
...
ready=0;for _ in $(seq 1 40);do if timeout 2 rosnode list 2>/dev/null|grep -q laserMapping&&timeout 2 rosnode list 2>/dev/null|grep -qE 'republish|image_transport';then ready=1;break;fi;kill -0 "$LAUNCH_PID" 2>/dev/null||break;sleep .5;done;[ "$ready" = 1 ]||{ FINAL_CLASS=PROCESS_LIFECYCLE_FAIL;FINAL_REASON="nodes not ready";exit 1;};write_state NODES_READY "" "nodes ready"
...
python3 "$PARITY_TOOL" --official "$CFG" "$CAMCFG" --snapshot "$RUN_DIR/effective_rosparams.after_launch.yaml" --out "$RUN_DIR/official_runtime_config_parity.json">"$RUN_DIR/parity.log" 2>&1||{ FINAL_CLASS=CONFIG_FAIL;FINAL_REASON="parity FAIL";exit 1;};write_state CONFIG_PARITY_PASS "" "parity PASS"
ls -la "$BAG">"$RUN_DIR/bag_info.txt"||{ FINAL_CLASS=INFRA_FAIL;FINAL_REASON="bag unreadable";exit 1;};write_state DATA_PATH_PREFLIGHT_PASS "" "bag present"
...
PY
write_state OUTPUT_VALIDATED "" "trajectory valid";[ "$EVAL_CMD" = : ]||bash -c "$EVAL_CMD">"$RUN_DIR/evaluation.txt" 2>&1||{ FINAL_CLASS=EVALUATOR_FAIL;FINAL_REASON="evaluator failed";exit 1;};write_state EVALUATED "" "evaluated";cp "$TRAJ" "$RUN_DIR/trajectory.tum";FINAL_STATE=SUCCESS;FINAL_CLASS="";FINAL_REASON="canonical result";EXPERIMENT_VALID=true
